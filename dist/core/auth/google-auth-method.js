...
export async function authorizeKiroGoogle(region) {
    ...
    try {
        ...
        if (!deviceAuthResponse.ok) {
            ...
            const error = new Error(`Google device authorization failed: ${deviceAuthResponse.status} ${errorText}`);
            throw error;
        }
        const deviceAuthData = await deviceAuthResponse.json();
        const { verification_url, verification_uri_complete, user_code, device_code, interval = 5, expires_in = 600 } = deviceAuthData;
...
export async function pollKiroGoogleToken(deviceCode, interval, expiresIn, region) {
    if (!deviceCode) {
        ...
        try {
            ...
            if (responseText) {
                try {
                    ...
                    throw new Error(`Google token polling failed: invalid JSON response (HTTP ${tokenResponse.status}): ${responseText.slice(0, 300)}`);
                }
            }
            if (tokenData.error) {
                ...
                if (errorType === 'expired_token') {
                    const error = new Error('Google device code has expired. Please restart the authorization process.');
                    throw error;
                }
                ...
                const error = new Error(`Google token polling failed: ${errorType} - ${tokenData.error_description || ''}`);
                throw error;
            }
            ...
            if (!tokenResponse.ok) {
                const error = new Error(`Google token request failed with status: ${tokenResponse.status} ${responseText ? `(${responseText.slice(0, 200)})` : ''}`);
                throw error;
            }
            throw new Error(`Google token polling failed: missing tokens in response: ${responseText ? responseText.slice(0, 300) : '[empty]'}`);
        }
        catch (error) {
            ...
            if (attempts >= maxAttempts) {
                const finalError = new Error(`Google token polling failed after ${attempts} attempts: ${error instanceof Error ? error.message : 'Unknown error'}`);
                throw finalError;
            }
        }
    }
...
export class GoogleAuthMethod {
    ...
    async authorize(inputs) {
        ...
        const oidcRegion = normalizeRegion(inputs?.region || this.config.idc_region);
        ...
        return {
            ...
            callback: async () => {
                try {
                    const token = await pollKiroGoogleToken(auth.deviceCode, auth.interval, auth.expiresIn, oidcRegion);
                    ...
                    throw new Error(`Google authorization failed: ${err.message}. Check ~/.config/opencode/kiro-logs/plugin.log for details.`);
                }
            }
        };
    }
