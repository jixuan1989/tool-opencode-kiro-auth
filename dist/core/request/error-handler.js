export class ErrorHandler {
    ...
    async handle(error, response, account, context, showToast) {
        ...
        if (response.status === 400) {
            ...
            if (reason.includes('CONTENT_LENGTH_EXCEEDS_THRESHOLD') ||
                ...
                if (context.reductionFactor > 0.05) {
                    ...
                    showToast(`Context too long. Retrying with ${Math.round(newFactor * 100)}% history...`, 'warning');
                    return {
                        shouldRetry: true,
                ...
                throw new Error('Context too long for Kiro API even after aggressive truncation. Please start a new conversation.');
            }
